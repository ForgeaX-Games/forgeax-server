...
- **Scene visibility**: when replacing a scene, preserve or replace camera and lighting intentionally. Lit materials with zero effective lights can render black without a fatal error. Inspect an actual game frame and relevant renderer/asset warnings; a running timer or visible HUD does not establish scene visibility. Keep rendering-prerequisite assertions when updating tests, or replace them with equivalent checks for an intentional unlit design. Use the installed Studio game-authoring skill for the focused verification steps.
