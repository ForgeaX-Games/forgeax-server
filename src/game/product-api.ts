...
/**
 * Detect whether a game dir is an interactive film-game (reel): its
 * `reel/scenarios.json` exists and has a non-empty `activeId` (the author has
 * activated a scenario in the reel extension). The package endpoint uses this
 * to route packaging to the reel bundler (a standalone film-game site) instead
 * of the Engine DevKit game build. Any read/parse failure is treated as
 * "not a reel game" (fall back to the existing ECS packaging path).
 *
 * Exported for tests (extension-package-reel).
 */
...
/** The active video-game scenario id of a game (video-game/scenarios.json
 *  `activeId`), or null when none is activated. Parallel to readReelScenarioId
 *  for the video-game fork; the studio preview mounts it via
 *  <GameVideoPlaySurface scenarioId=...>. */
export function readGameVideoScenarioId(gameDir: string): string | null {
  try {
    const p = join(gameDir, 'video-game', 'scenarios.json');
    if (!existsSync(p)) return null;
    const db = JSON.parse(readFileSync(p, 'utf-8')) as { activeId?: unknown };
    return typeof db.activeId === 'string' && db.activeId.length > 0 ? db.activeId : null;
  } catch {
 ...
 * game's `forge.json` (`projectType: "reel"` is stamped by the reel
 ...
 */
export function readProjectType(gameDir: string): 'reel' | 'engine' | 'video-game' {
  try {
    const m = JSON.parse(readFileSync(join(gameDir, 'forge.json'), 'utf-8')) as {
      projectType?: unknown;
    };
    if (m.projectType === 'reel' || m.projectType === 'engine' || m.projectType === 'video-game') {
...
function resolveEngineGameTemplate(slug: string): string | null {
  ...
  try {
    ...
    const manifestPath = join(templateDir, 'forge.json');
    if (!statSync(manifestPath).isFile()) return null;
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf-8')) as { entry?: unknown };
    if (typeof manifest.entry !== 'string' || manifest.entry.trim().length === 0 || isAbsolute(manifest.entry)) return null;
    const entryPath = resolve(templateDir, manifest.entry);
    if (!entryPath.startsWith(`${templateDir}${sep}`) || !statSync(entryPath).isFile()) return null;
    return templateDir;
  } catch {
    return null;
  }
...
function ledgerFilesByAgent(
  ...
  for (const rec of records) {
    ...
    const pathAgentId = rec.agentPath.split('/').pop() || rec.agentPath;
    const agentId = pathAgentId === 'root' && mainAgentId ? mainAgentId : pathAgentId;
...
async function listAllGames(root: string): Promise<Array<{ slug: string; name: string; mtime: number; fileCount: number; projectType: 'reel' | 'engine' | 'video-game'; reelScenarioId: string | null }>> {
  ...
  const out: Array<{ slug: string; name: string; mtime: number; fileCount: number; projectType: 'reel' | 'engine' | 'video-game'; reelScenarioId: string | null }> = [];
  try {
    for (const slug of readdirSync(gamesDir)) {
      ...
      // Decision B: declared project type from forge.json (reel / video-game
      // preview routing). `reelScenarioId` carries the active scenario id for
      // BOTH reel and the video-game fork (the studio reads it generically).
      const projectType = readProjectType(dir);
      const reelScenarioId = projectType === 'video-game' ? readGameVideoScenarioId(dir) : null;
...
export interface ProductApiRouterOptions {
...
export function createProductApiRouter(options: ProductApiRouterOptions = {}): Hono {
  ...
  router.put('/projects/active', async (c) => {
  ...
  // This is the extension-scoped projection of the same catalog exposed at
  ...
  router.delete('/projects/:slug', async (c) => {
  ...
  // AgentsPanel + PageView poll at 4 s with include=files, which walks
  ...
  // `packages/marketplace`) are viewable/editable in agent-persona. The
  ...
  const resolvePersonaLanguagePath = async (raw: string, lang: 'zh' | 'en', forWrite: boolean) => {
    const cands = [raw, raw.replace(/^@[^/]+\//, ''), raw.replace(/^@[^/]+\//, '').replace(/^agent-/, '')];
    try {
      const { config, packDir } = loadBrand();
      if (cands.includes(config.assistant.agent.id)) {
        const relative = config.assistant.agent.personaFiles[lang]
          ?? config.assistant.agent.personaFiles.zh;
        return resolve(packDir, relative);
      }
    } catch {
      // Brand resolution errors fall through to extension lookup so a broken
      // product pack does not hide independently installed agent personas.
    }
    const persona = await resolvePersonaAny(raw);
  ...
  router.get('/agents/:id/persona', async (c) => {
    ...
    const path = await resolvePersonaLanguagePath(id, lang, false);
    if (!path) return c.json({ error: `agent not found: ${id}` }, 404);
    try {
  ...
  router.put('/agents/:id/persona', async (c) => {
    ...
    const target = await resolvePersonaLanguagePath(id, lang, true);
    if (!target) return c.json({ error: `agent not found: ${id}` }, 404);
    try {
  ...
  router.get('/agents', async (c) => {
    ...
    let config: ReturnType<typeof loadBrand>['config'];
    try {
      config = loadBrand().config;
    } catch (e) {
    ...
    if (includeFiles && sid) {
      try {
        ...
        if (session) {
          ...
          ledgerByAgent = ledgerFilesByAgent(records, projectRoot, config.assistant.agent.id);
        }
    ...
    try {
      ...
      const extensionManifests = getExtensionSnapshot().manifests;
      const descriptionByAgentId = new Map<string, { zh?: string; en?: string } | string>();
      for (const entry of extensionEntries) {
        const merged = extensionManifests.find((m) => m.manifest.id === entry.extensionId);
        const raw = merged?.manifest.description;
        if (raw) descriptionByAgentId.set(entry.definition.id, raw);
      }
      descriptionByAgentId.set(config.assistant.agent.id, {
        zh: '把一句话想法拆成任务并派给对口队友，自己盯全局验收；引擎游戏代码可亲自写。',
        en: 'Turns a one-line idea into tasks, delegates to the right teammates, and owns acceptance. Engine-game code can be written in-house.',
      });
      const descriptionFor = (id: string): string | undefined => {
        const raw = descriptionByAgentId.get(id);
        if (!raw) return undefined;
        const text = pickI18n(raw, lang);
        return text.trim() || undefined;
      };
      type CardT = (typeof extensionEntries)[number]['definition']['card'];
      const namingForCard = (card: CardT, fallbackName: string): { naming: AgentNaming; personName?: string } => {
      ...
      const mainId = config.assistant.agent.id;
      const mainName = config.assistant.cardName?.[lang]
        ?? config.assistant.cardName?.zh
        ?? config.assistant.cardName?.en
        ?? config.assistant.name;
      const mainPersonName = config.assistant.name;
      const agents: Array<Record<string, unknown>> = [{
        ...
        naming: computeAgentNaming({
          personName: mainPersonName,
          cnTitle: config.assistant.cardName?.zh,
          enTitle: config.assistant.cardName?.en,
          fallback: mainName,
        }, lang),
        role: 'orchestrator',
        color: '#A8E6B8',
        avatar: config.assistant.avatarSrc ?? mainPersonName[0]?.toUpperCase() ?? 'F',
        ...(descriptionFor(mainId) ? { description: descriptionFor(mainId) } : {}),
        status: 'active',
        isMain: true,
        files: !includeFiles ? [] : (ledgerByAgent?.get(mainId) ?? []),
      }];
      const seenIds = new Set([mainId]);
      for (const entry of extensionEntries) {
        ...
        const { naming, personName } = namingForCard(def.card, name);
        const description = descriptionFor(def.id);
        agents.push({
          id: def.id,
          name,
    ...
    try {
      ...
      await cp(templateDir, gameDir, {
        ...
        filter: (src) => {
          ...
          return b !== 'sessions' && b !== 'node_modules' && b !== '.git' && b !== '.forgeax';
        },
      });
  ...
  router.post('/projects/link', async (c) => {
  ...
  router.post('/projects/:slug/verify', async (c) => {
    const slug = c.req.param('slug');
    if (!GAME_SLUG_RE.test(slug ?? '')) {
      return c.json({ error: 'invalid slug' }, 400);
    }
  ...
  router.post('/projects/:slug/builds', async (c) => {
    const slug = c.req.param('slug');
    if (!GAME_SLUG_RE.test(slug ?? '')) {
      return c.json({ error: 'invalid slug' }, 400);
    }
  ...
  router.get('/project-builds/jobs/:id', (c) => {
    const job = getJob(c.req.param('id'));
    if (!job) return c.json({ error: 'job not found' }, 404);
    return c.json(job);
  });

  // ── GET /project-builds/engine-roots — detect Engine workspaces for DevKit builds ──
  router.get('/project-builds/engine-roots', (c) => {
    return c.json({ roots: detectEngineRoots(resolve(assetRoot(), '..')) });
  });

  // ── GET /package/history — list packaging history ──
  router.get('/project-builds/history', (c) => {
    return c.json({ records: listHistory() });
  });

  // ── POST /package/clean — wipe local packaging environment ──
  // Removes the isolated Rust toolchain and launcher-shell cache. Does NOT
  // touch export products or history.
  router.post('/project-builds/clean', (c) => {
    return c.json(cleanPackagingEnv());
  });

  // ── DELETE /package/history/:id — delete a single history record ──
  router.delete('/project-builds/history/:id', (c) => {
  ...
  router.post('/project-builds/reveal', async (c) => {
    let target = '';
  ...
  router.get('/project-play/:slug/*', async (c) => {
    const slug = c.req.param('slug');
  ...
  // Polled every 4s by PageView.AgentsMainArea ledger panel. Each call
  ...
  router.get('/agents/events/recent', async (c) => {
    const lang = (c.req.query('lang') ?? 'en') as 'zh' | 'en';
    ...
    for (const entry of listAgents()) {
      const def = entry.definition;
      const files = await resolveProduces(projectRoot, def.produces ?? [], undefined);
      const cardName = def.card.name;
      const agentName = typeof cardName === 'string'
        ? cardName
        : (cardName[lang] ?? cardName.zh ?? cardName.en ?? def.id);
      for (const f of files) {
        try {
          ...
          events.push({
            name: f.name, path: f.path, agentId: def.id,
            agentName,
            mtime: st.mtimeMs,
            ico: f.ico,
          });
