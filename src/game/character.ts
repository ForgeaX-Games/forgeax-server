 ...
 * character/server/character-forge/`,编排层不再持有任何 character 业务。
 ...
 *   - GET/POST /active-character  跨扩展页「当前角色」指针(纯指针,非 character 业务)
 *
 * dual-modality:每次成功代理后 dispatchToSurface('character.host','reload',…)
...
import { callTool, dispatchToSurface } from '@forgeax/orchestrator';
...
// packages/marketplace/extensions/character-host/panel.tsx — 'character.host'.
const SURFACE_ID = 'character.host';
...
export function createCharacterRouter(ctx: CharacterRouterCtx): Hono {
  ...
  // ── active-character pointer(通用跨扩展页指针,非 character 业务)──────────
...
function safeAssetPath(root: string, rel: string): string | null {
  ...
  // per-game asset root. Every extension shares ONE validated write path.
  const ALLOWED_MODULES = new Set([
    'characters', 'skills', 'reel', 'video-game', 'narrative', 'lowpoly-characters', 'public', 'scene',
  ]);
