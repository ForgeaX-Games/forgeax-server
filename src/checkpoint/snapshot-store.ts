...
export class SnapshotStore {
  ...
  private readonly statCache = new Map<string, { mtimeMs: number; size: number; h: string }>();
  ...
  private _walk(root: string, dir: string, out: Record<string, ManifestEntry>): void {
    ...
    for (const name of entries) {
      ...
      if (cached && cached.mtimeMs === st.mtimeMs && cached.size === st.size) {
        ...
        this.statCache.set(abs, { mtimeMs: st.mtimeMs, size: st.size, h });
      }
      // stat 缓存命中时 blob 必然已存在(同一进程内写过);进程重启后缓存为空,
      // 首次 walk 会重哈希并按需补 blob,所以这里不需要二次 existsSync。
      out[rel] = { h, size: st.size, mode: st.mode & 0o777 };
    }
  }
  ...
  restore(
    ...
    for (const [rel, entry] of Object.entries(manifest.files)) {
      ...
      try { chmodSync(abs, entry.mode); } catch { /* 非关键 */ }
  ...
  diffDiskVsManifest(targetDir: string, manifest: Manifest): {
