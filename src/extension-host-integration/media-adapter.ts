...
function requestContext(
  ...
  return {
    gameId,
    identity: options.identity?.(gameId) ?? `extension-host:${gameId}`,
...
function publicUrl(gameId: string, runtimeId: string, assetId: string): string {
  return `/__extensions__/v1/extension/${encodeURIComponent(runtimeId)}/media/resources/${encodeURIComponent(assetId)}/content?gameId=${encodeURIComponent(gameId)}`;
}
...
function toAsset(
  ...
  const extra = Object.fromEntries(
    Object.entries(resource.source_meta?.extra ?? {})
      .filter(([key]) => key !== 'extension_fingerprint'),
  );
...
function resourceId(key: string): string {
  return `extension-${createHash('sha256').update(key).digest('hex').slice(0, 32)}`;
}
  ...
  const capability: ResumableMediaCapability = {
    ...
    async put(gameId, input): Promise<MediaAsset> {
      ...
      if (stableId) {
        try {
          ...
          if (existing.source_meta?.extra?.extension_fingerprint !== digest) {
    ...
    async update(
      ...
      try {
        ...
        const fingerprint = existing.source_meta?.extra?.extension_fingerprint;
        const nextExtra = input.metadata === undefined
          ? existing.source_meta?.extra
          : {
              ...input.metadata,
              ...(typeof fingerprint === 'string' ? { extension_fingerprint: fingerprint } : {}),
            };
