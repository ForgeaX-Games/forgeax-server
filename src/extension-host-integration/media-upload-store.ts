...
function assertStoredUpload(value: unknown): asserts value is StoredMediaUpload {
  if (!isRecord(value)) throw new Error('Extension media upload checkpoint is invalid');
  if (
    value.version !== 1
    ...
    throw new Error('Extension media upload checkpoint is invalid');
  }
}
...
async function readBytesNoFollow(path: string, expectedSize: number): Promise<Uint8Array> {
  ...
  try {
    ...
    if (!info.isFile() || info.size !== expectedSize) {
      throw new Error('Extension media upload bytes are invalid');
    }
...
/** Durable, game-scoped checkpoints for the Extension resumable media contract. */
export class ForgeaxMediaUploadStore {
  ...
  async #read(
    ...
    if (value.id !== uploadId || value.gameId !== gameId) {
      throw new Error('Extension media upload checkpoint identity is invalid');
    }
