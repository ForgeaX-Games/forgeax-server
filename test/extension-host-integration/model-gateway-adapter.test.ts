...
} from '../../src/extension-host-integration/model-gateway-adapter';
...
describe('createForgeaxModelGateway', () => {
  ...
  test('resolves game-owned references and persists generated image bytes', async () => {
    ...
    const provider: ForgeaxModelProvider = {
      ...
      generateVideo: async () => { throw new Error('must not reach video provider'); },
    };
  ...
  test('does not fall back to the direct ForgeaX video provider', async () => {
    let called = false;
    const provider: ForgeaxModelProvider = {
      generateText: async () => { throw new Error('unused'); },
      generateImage: async () => { throw new Error('unused'); },
      generateVideo: async () => {
        called = true;
        throw new Error('direct provider must not be called');
      },
    };
    const gateway = createForgeaxModelGateway(provider, memoryMedia());

    await expect(gateway.generateVideo('game-1', { prompt: 'clip' }))
  ...
  test('rejects external model reference URLs', async () => {
    ...
    await expect(gateway.generateImage('game-1', {
      prompt: 'frame',
      references: [{ url: 'https://example.invalid/image.png' } as never],
