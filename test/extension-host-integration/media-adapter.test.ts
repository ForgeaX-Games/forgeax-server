...
import { createForgeaxMediaCapability, type ForgeaxVideoAssetService } from '../../src/extension-host-integration/media-adapter';
...
describe('createForgeaxMediaCapability', () => {
  ...
  test('maps list, local reads, uploads, retries, and delete through the existing service', async () => {
    ...
    const service: ForgeaxVideoAssetService = {
      ...
      prepareUpload: async (input) => {
        ...
        return {
          ...
          object_url: 'extension-upload:token',
          upload_token: 'token',
        };
      },
    ...
    expect(listed[0]).toMatchObject({
      ...
      url: '/__extensions__/v1/extension/rt-video/media/resources/resource-1/content?gameId=game-1',
    });
