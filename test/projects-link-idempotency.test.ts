...
describe('POST /api/projects/link', () => {
  test('re-opening the mounted game is idempotent without changing selection', async () => {
    ...
    const response = await app.request('/api/projects/link', {
      method: 'POST',
  ...
  test('activates a linked editor-owned sample outside the games consumer checkout', async () => {
    ...
    try {
      const linkResponse = await app.request('/api/projects/link', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ path: gameDir }),
      });
      ...
      const activateResponse = await app.request('/api/projects/active', {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
