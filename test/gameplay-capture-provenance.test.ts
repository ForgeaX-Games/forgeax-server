...
describe('gameplay capture provenance', () => {
  test('artifact is readable and stale provenance is rejected before producer or focus', async () => {
    ...
    const adapter = new CarrierGameplayAdapter({ supervisor: { ensure: async () => snapshot, status: async () => snapshot } as never, gateway: { execute: async (request) => request.operation.operation === 'capture' ? { ok: true, operation: 'capture', state: 'running', identity, data: await capture.produce(identity) } : { ok: true, operation: request.operation.operation, state: 'running', identity } } });
